...
// 定义一组错误变量，每个变量通过 xerror.NewError 创建，用于标识不同的错误场景和错误信息。
...
var (
	...
	ErrTooManyLoginError       = xerror.NewError(20013, "登录失败次数过多，请稍后再试")
	ErrNicknameLengthLimit     = xerror.NewError(20014, "昵称长度2~12")
	ErrNoExistUsername         = xerror.NewError(20015, "用户不存在")
	ErrNoAdminPermission       = xerror.NewError(20016, "无管理权限")
	ErrDisallowUserRegister    = xerror.NewError(20017, "系统不允许注册用户")
	ErrOauthWrong              = xerror.NewError(20018, "统一验证密码错误")
