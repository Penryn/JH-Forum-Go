...
func (s *DaoServant) PrepareTweets(userId int64, tweets []*ms.PostFormated) error {
	...
	for _, tweet := range tweets {
		if tweet.User != nil {
			// tweet.User.IsFriend, tweet.User.IsFollowing = friendMap[tweet.UserID], followMap[tweet.UserID]
			if following, exists := followMap[tweet.UserID]; exists {
				tweet.User.IsFollowing = following
			} else {
				// 处理 followMap 中不存在该键的情况
				tweet.User.IsFollowing = false // 或其他默认值
			}
		} else {
			// 处理 tweet.User 为空的情况
			logrus.Errorf("PrepareTweets' tweet.User is nil")
		}
	...
	if err := s.Redis.SetPushToSearchJob(ctx); err == nil {
		...
		for {
			...
			for i, pf := range postsFormated {
				...
				fmt.Println("push to search: ", docs)
